...
class Report
  ...
  def initialize(template_name, &block)
    ...
    @file = DOCXReport::File.new(template_name)
  ...
  def generate(dest = nil, filename = nil, &block)
    ...
    @file.update('document.xml', 'header1.xml', 'footer1.xml', 'header2.xml', 'footer2.xml', 'header3.xml', 'footer3.xml') do |txt|
    ...
    if block_given?
      ...
      @file.remove
		...
		if filename
			@new_path = @file.path[0,@file.path.rindex('/')+1]+filename.gsub(/[^0-9A-Za-z.\-]/, '_')
...
private
  ...
  def parse_document(txt)
    ...
    txt.replace(doc.to_s)
		#txt.replace(doc.to_xml(:save_with => Nokogiri::XML::Node::SaveOptions::AS_XML).strip.gsub("'","&apos;"))
  end
